...
function Update()
	...
	for i = 0, nLines do
		...
		local y0 = (RadiusY + mband0 * math.pow(mband0, Squared) * (-Inward * 2 + 1) * ExtrudeMax) * 
			math.sin((AngleTotal - AngleTotal / NumOfLines * a + AngleStart) % PIx2) + RadiusY + ExtrudeMax
		...
		x0 = (Radius + mband0 * math.pow(mband0, Squared) * (-Inward * 2 + 1) * ExtrudeMax) * 
			math.cos((AngleTotal - AngleTotal / NumOfLines * a + AngleStart) % PIx2) + Radius + ExtrudeMax
		y0 = (RadiusY + mband0 * math.pow(mband0, Squared) * (-Inward * 2 + 1) * ExtrudeMax) * 
			math.sin((AngleTotal - AngleTotal / NumOfLines * a + AngleStart) % PIx2) + RadiusY + ExtrudeMax
